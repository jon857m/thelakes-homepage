...
// - Open-Meteo Geocoding (no key) for search
// - Buttons: Use my preferences + Use my location
// - DOES NOT auto-load preference (deliberate choice UX)
// - Uses simple daily R/A/G thresholds
// ========================================
...
if (typeof window.enableDragScroll !== "function") {
  ...
  function dayRisk(d) {
    ...
    const gust = Number(d.gustMax ?? 0);
    const mm = Number(d.rainMm ?? 0);

    // Red: clearly poor for most plans
    if (pp >= 70 || gust >= 45 || mm >= 15) return { cls: "red", label: "Red" };

    // Amber: mixed / caution
    if (pp >= 40 || gust >= 30 || mm >= 5) return { cls: "amber", label: "Amber" };

    return { cls: "green", label: "Green" };
  }

  function fmtDate(iso) {
    // 2026-02-17 -> Tue 17 Feb
...
function renderDaily(loc, data) {
  ...
  // Reuse the Fell Planner rail style (6 rows + icons)
  ...
  btnUsePrefs?.addEventListener("click", () => {
    ...
    // your Snapshot saves { name/place, lat, lon, ... } :contentReference[oaicite:4]{index=4}
  ...
  btnUseDevice?.addEventListener("click", () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        setLocation({ name: "My location", lat, lon, source: "Device" });
      },
      () => {
  ...
  // Blank by default (do nothing else)
  setViewing(null);
  setStatus("Choose a location to begin.");
})();
